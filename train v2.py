...
from loaders import FolderLoader, ClassLoaderFromDict
import pandas
...
parser.add_argument('--model', default=None, choices=list(AVAILABLE_KERAS_MODELS.keys()))
...
def load_data():
    data_loader = FolderLoader(sample_rate=args.sample_rate, window=args.window, step=args.step, class_loader=ClassLoaderFromDict({item['fname']: item['labels'] for idx, item in pandas.read_csv("/media/dejavu/Seagate Portable/Freesound/train_curated.csv").iterrows()}), out_folder=args.folder, use_mmap=False)
