...
class AudioModelBuilder:
    ...
    def __get_simple_MNIST_convnet(self, num_classes):
        model = Sequential()
        model.add(layers.Input(shape=(self.sample_rate*self.context_window, self.n_mels, 1)))
    ...
    def __pretrained_model_with_conv(self, num_classes, pretrained_model=MobileNetV3Small):
        input_tensor = layers.Input(shape=(get_mels_from_hop_and_win_lengths(self.stft_hop, self.stft_window, input_size=self.sample_rate*self.context_window), self.n_mels, 1))
    ...
    def get_melspectrogram(self):
        return get_melspectrogram_layer(
            ...
            input_shape=(self.sample_rate * self.context_window, 1)
        )
...
if __name__ == '__main__':
    ...
    # Show single item
    plt.matshow(melspectrograms[test_item_index].squeeze())
    plt.show()
    print(melspectrograms.min())
    for melspectrogram in melspectrograms:
        plt.matshow(melspectrogram.squeeze(), vmin=-50, vmax=5)
        plt.show()
