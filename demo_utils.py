...
def draw_history(history, metric, smooth=.8, alpha=0.25):
    ...
    ax.set_xlabel('Epoch')
    max_val_metric_epoch = np.argmax(history[val_metric])
    max_val_metric = history[val_metric][max_val_metric_epoch]
    ax.annotate(f'{round(max_val_metric * 100, 2)}%  at epoch {max_val_metric_epoch}',
                xy=(max_val_metric_epoch, max_val_metric),
                xycoords='data',
                xytext=(0.5, 0.5),
    ...
    ax.scatter(max_val_metric_epoch, max_val_metric, color=val_metric_color, marker='x', s=100)
...
def get_image_from_history(history, metric):
    ...
    """
    return buffer_plot_and_get(draw_history(history, metric))
...
if __name__ == '__main__':
    ...
    get_image_from_history(history, 'loss').show()