...
def __window(a, window=DEFAULT_WINDOW, step=DEFAULT_STEP, sr=SAMPLE_RATE) -> np.ndarray:
    w = int(window * sr)
    o = int(step * sr)
    sh = (a.size - w + 1, w)
    st = a.strides * 2
...
def apply_window(audio: Union[AudioSegment, np.array], window=DEFAULT_WINDOW, step=DEFAULT_STEP, sr=SAMPLE_RATE):
...
def get_mels_from_hop_and_win_lengths(hop_length, win_length, input_size=SAMPLE_RATE*DEFAULT_WINDOW):
